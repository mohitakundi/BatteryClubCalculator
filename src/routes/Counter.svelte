<script>
	...
	// Display the object where Company = 'Eveready'
...
<div class="counter">
	<button on:click={() => (wattage -= 0.5)} aria-label="Decrease the counter by one">
	...
	<div class="counter-viewport">
		<div class="counter-digits" style="transform: translate(0, {200 * offset}%)">
			<strong class="hidden" aria-hidden="true">{Math.floor($displayed_count)+0.5}</strong>
			<strong>{Math.floor($displayed_count)}</strong>
		</div>
	</div>

	<button on:click={() => (wattage += 0.5)} aria-label="Increase the counter by one">
		<svg aria-hidden="true" viewBox="0 0 1 1">
		...
		<div class="counter-digits" >
			<p>
				...
				{#if batteryLife !== Infinity}
					{Math.floor(batteryLife)}<!-- Display if battery life is not INFINITY -->
				{:else}
					Enter a wattage
				{/if}
			</p>
